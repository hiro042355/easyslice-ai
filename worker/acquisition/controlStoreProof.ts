...
import { ACQUISITION_CONTROL_PREFIX, acquisitionControlObjectName, PersistentAcquisitionIdempotencyStore,
  type AcquisitionControlRecord } from "../../lib/server/acquisitionWorker/persistentIdempotency";
import { createAdcAccessTokenSupplier, GcsAcquisitionControlObjectStore,
  readAcquisitionControlConfiguration } from "../../lib/server/acquisitionWorker/gcsControlStore";
...
const terminal = (record: Extract<AcquisitionControlRecord, { state: "running" }>): AcquisitionControlRecord => Object.freeze({
  schemaVersion: "1.0", acquisitionId: record.acquisitionId, requestFingerprint: record.requestFingerprint,
  state: "failed", fenceToken: record.fenceToken, createdAt: record.createdAt,
  updatedAt: new Date().toISOString(), result: failure(record.acquisitionId),
});
const objectUrl = (bucket: string, name: string): string =>
...
export const runProductionControlStoreProof = async (): Promise<ControlStoreProofEvidence> => {
  const configuration = readAcquisitionControlConfiguration();
  const productionMode = configuration.mode === "PRODUCTION";
  const productionBucket = configuration.bucket === "nexcut-prod-jp-2026-media";
  const controlPrefixExact = configuration.prefix === ACQUISITION_CONTROL_PREFIX;
  let experimentRejected = false;
  try {
    readAcquisitionControlConfiguration({ ACQUISITION_CONTROL_MODE: "EXPERIMENT",
      ACQUISITION_CONTROL_BUCKET: configuration.bucket, ACQUISITION_EXPERIMENT_BUCKET: configuration.bucket });
  } catch { experimentRejected = true; }

  const token = createAdcAccessTokenSupplier();
  await token.getAccessToken();
  const ambientAuth = true;
  const objects = new GcsAcquisitionControlObjectStore(configuration, token);
  ...
  let staleTakeover = false, oldOwnerFenced = false, terminalWriteFenced = false, leaseAbort = false;
  let mediaPrefixDenied = false;
  try {
    ...
    if (expiredCreate.status === "created") {
      ...
      const takeoverResult = await objects.replace(names[3], expiredCreate.generation, takeover);
      staleTakeover = takeoverResult.status === "updated";
      oldOwnerFenced = (await objects.replace(names[3], expiredCreate.generation, expired)).status === "precondition-failed";
      terminalWriteFenced = (await objects.replace(names[3], expiredCreate.generation, terminal(expired))).status === "precondition-failed";
    }
    ...
    const denied = await fetch(objectUrl(configuration.bucket, `jobs/${randomUUID()}/input/control-proof-denied`), {
      headers: { authorization: `Bearer ${await token.getAccessToken()}` },
    });
    ...
    for (const name of names) {
      ...
      if (current.status === "found") {
        await fetch(`${objectUrl(configuration.bucket, name)}?ifGenerationMatch=${current.object.generation}`, {
          method: "DELETE", headers: { authorization: `Bearer ${await token.getAccessToken()}` },
        });
      }
    }
  }
  ...
  return Object.freeze({ ambientAuth, productionMode, productionBucket, experimentRejected, controlPrefixExact,
    controlCreate, concurrentClaim, controlRead, sameFingerprintReplay, differentFingerprintRejected, casUpdate,
    staleCasRejected, heartbeat, staleTakeover, oldOwnerFenced, terminalWriteFenced, leaseAbort, mediaPrefixDenied,
    listingCallCount: 0, cleanup: testControlResidue === 0, testControlResidue });
};
